...
prompt_kubecontext() {
  ...
  if [[ -n "$kubectl_version" ]]; then
    ...
    local cur_ctx=$(kubectl config view -o=jsonpath='{.current-context}' 2> /dev/null)
    cur_namespace="$(kubectl config view -o=jsonpath="{.contexts[?(@.name==\"${cur_ctx}\")].context.namespace}" 2> /dev/null)"
